...
#ifndef CGAL_LINF1D_VORONOI_DIAGRAM_TRAITS_2_H
#define CGAL_LINF1D_VORONOI_DIAGRAM_TRAITS_2_H
...
class Linf1D_voronoi_traits_2 : public Arr_linear_traits_2<Kernel_>
{
public:
  ...
  //L1_voronoi_traits
  typedef Linf1D_voronoi_traits_2<Kernel>          Self;
  ...
  static Comparison_result compare_z_at_xy (const X_monotone_curve_2& cv,
    ...
    std::cout << "in compare_z_at_xy cv = " << cv << ", h1 = " << h1 << ", h2 = " << h2 << ", above = " << above << ", side = "  << side << std::endl;
    ...
    if ((l.is_vertical())){ 
     ...
     side = CGAL::opposite(side);
      ...
      std::cout << "compare_x_at_y(h1, l) = " << res <<std::endl;
      std::cout << "compare_x_at_y(h2, l) = " << compare_x_at_y(h2, l) <<std::endl;
     ...
     //(which are parts of 2d bisector). In that case it returns CGAL::EQUAL
        //if((h1.x() == -100) && (h2.x() == 250)){
                 // std::cout << "turned the result";
...
//here 1D bisector is constructed: in case 2 input points have same x or y coordinate. 
//Also the bisector consists of a single line when inpur points lie in the corners of a square. 
      //ToDo:check this
      if((CGAL::abs(delta_x) == CGAL::abs(delta_y))){
     //||(s_x == CGAL::ZERO || s_y == CGAL::ZERO ))
      ...
      if (cmppy == CGAL::LARGER) {
        ...
        if(s_x == CGAL::ZERO){
          ...
          std::cout << "s_x == 0" << " vertical bisector" << std::endl;
          *o++ = CGAL::make_object(Intersection_curve(Segment_2(p1, p2), 1));
          *o++ = CGAL::make_object(Intersection_curve(Ray_2(*top, Direction_2(0, 1)), 1));
          *o++ = CGAL::make_object(Intersection_curve(Ray_2(*bottom, Direction_2(0, -1)), 1));
          return o;
        }
  ...
  //      mult = 1;
   //   }
      ...
      if (cmppx != CGAL::EQUAL) {
        ...
        if  (((s_x == CGAL::POSITIVE) and (CGAL::compare(s2.y(), p1.y()) == CGAL::LARGER))|| ((s_x == CGAL::NEGATIVE)and(CGAL::compare(s1.y(), p1.y()) == CGAL::LARGER))){
          topright = true;
        }

        if  (((s_x == CGAL::POSITIVE) and (CGAL::compare(s1.y(), p1.y()) == CGAL::LARGER))||((s_x == CGAL::NEGATIVE) and (CGAL::compare(s2.y(),p1.y()) == CGAL::LARGER))){
          topleft = true;
        }
      ...
      if (cmppy != CGAL::EQUAL) {
        ...
        if  (((s_y == CGAL::POSITIVE) and (CGAL::compare(s2.x(), p1.x()) == CGAL::LARGER))|| ((s_y == CGAL::NEGATIVE)and(CGAL::compare(s1.x(), p1.x()) == CGAL::LARGER))){
                topright = true;
        } 

        if  (((s_y == CGAL::POSITIVE) and (CGAL::compare(s1.x(), p1.x()) == CGAL::LARGER))||((s_y == CGAL::NEGATIVE) and (CGAL::compare(s2.x(),p1.x()) == CGAL::LARGER))){
                topleft = true;
        } 
